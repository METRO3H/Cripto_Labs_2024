...
function Analyze_Web_Page() {
  ...
  for (let i = 0; i < encrypted_messages.length; i++) {
    console.log(`Mensaje ${i + 1} : \n${encrypted_messages[i]} -> ${decrypted_messages[i]}`);
  }

  Show_Decrypted_Messages_On_Screen(encrypted_messages, decrypted_messages);
}
...
function Show_Decrypted_Messages_On_Screen(encrypted_messages, decrypted_messages) {
